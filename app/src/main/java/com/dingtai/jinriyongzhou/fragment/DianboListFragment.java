...
import com.dingtai.jinriyongzhou.activity.DianBoListActivity;
...
public class DianboListFragment extends BaseFragment {
    ...
    private final static int API_FLAG = IndexAPI.DIANBO_LIST_API;
    private final static String MESSAGE_FLAG = IndexAPI.DIANBO_LIST_API_MESSAGE;
...
//        String url = API.COMMON_URL + "/Interface/NewsChildAPI.ashx?action=GetNewsChannelByPolitics&ChId=" + id + "&StID=" + API.STID + "&top=10";
        String url = API.COMMON_URL + "interface/VodByJRYZ.ashx?action=GetVodClass&top=10&dtop=0&STid=0";
            ...
            switch (msg.what) {
                case API_FLAG:
                    ...
                    if (!msg.obj.toString().equals("暂无更多数据")) {
                        ArrayList<VodClassListBean> tempList = (ArrayList<VodClassListBean>) msg.getData().getParcelableArrayList("list").get(0);
...
//            return new SecondHolder(LayoutInflater.from(getActivity()).inflate(R.layout.item_paper_net, parent, false));
            return new DefaultHolder(LayoutInflater.from(getActivity()).inflate(R.layout.item_guangdian_zhibo, parent, false));
        }
    ...
    private class DefaultHolder extends BaseHolder {
        ...
        public void onItemClick(int position) {
            Intent intent = new Intent(getActivity(), DianBoListActivity.class);
            intent.putExtra("ID", mDatalist.get(position).getID());
            intent.putExtra("title", mDatalist.get(position).getVodClassName());
            startActivity(intent);
        }

        public void onbind(int position) {
            int newPosition = position;
            VodClassListBean itemBean = mDatalist.get(newPosition);
            if (itemBean != null) {
                ...
                ImgTool.getInstance().loadRoundImg(itemBean.getVodClassLogo(), mLogoIv, 5);

                if (!TextUtils.isEmpty(itemBean.getVodClassName()))
                    mTitleTv.setText(itemBean.getVodClassName());
                if (!TextUtils.isEmpty(itemBean.getRemark()))
                    mTitleTv.setText(itemBean.getRemark());
