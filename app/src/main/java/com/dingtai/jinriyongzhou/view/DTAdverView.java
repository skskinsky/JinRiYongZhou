...
import com.dingtai.jinriyongzhou.adapter.JDViewAdapter;
...
public class DTAdverView extends LinearLayout {
    ...
    public DTAdverView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DTAdverView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.DTAdverView);
        mAdverHeight = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, jdAdverHeight, getResources().getDisplayMetrics());
        int gap = array.getInteger(R.styleable.DTAdverView_gap, mGap);
        int animDuration = array.getInteger(R.styleable.DTAdverView_animDuration, mAnimDuration);
